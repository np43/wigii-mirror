...
/**
 * this class is the main service which will do the glue between the GUI and the API
...
//");
	...
	/**
	 * define each action on which we need to not send other information before and after
	 * the specific action
	 */
...
";
	...
	protected function doAutoLoginIfNeeded() {
		...
		// trie auto login using credentials cookie
		$returnValue = $authS->autoLogin();
		// if not successfull, trie auto login as public user
		if(!$returnValue &&
	...
	//if obtaining lock faild -> display message return false
...
//		eput($_FILES);
	...
	//when manipulating the indicatorList you always need to go through this method to be sure
	//to always get the unique one.
...
//					fput($logExp);
	 ...
	 * exec: The execution service for this request
	 * syscons: the system console service
	 */
...
//					$groupAS->getAllGroups($p, $exec->getCrtModule(), $groupList, $groupAS->getListFilterForSelectRootGroupsWithoutDetail($exec->getCrtModule()));
			...
			case "simpleFilters": //when only the input text, (no reset of others filters)
				...
				if (!isset ($groupAS)) $groupAS = ServiceProvider :: getGroupAdminService();
				if (!isset ($elS)) $elS = ServiceProvider :: getElementService();
				$totalWidth = 450; //$configS->getParameter($p, $exec->getCrtModule(), "addIndicatorTotalWidth");
				$labelWidth = 150; //$configS->getParameter($p, $exec->getCrtModule(), "addIndicatorLabelWidth");;
				...
				if($selectSearchBarActivityName){
					...
					$configS->getFields($p, $exec->getCrtModule(), Activity::createInstance($selectSearchBarActivityName), $fssbf);
				...
				if($exec->getCrtAction() !="simpleFilters"){
					$limitFilterInGroup = $filtersRec->getFieldList()->getField("limitFilterInGroup");
					if($configS->getParameter($p, $exec->getCrtModule(), "Group_selectAllGroupsOnSearch") != "0"){
						...
						$groupPTreeArrayImpl = GroupPListTreeArrayImpl :: createInstance();
						$inGroupModuleXml = '<limitFilterInGroup type="Attributs">';
						$inGroupModuleXml .= '<attribute><label>--' . $transS->t($p, "rootGroup") . '--</label></attribute>';
						//get all group, only admin
						$groupPTreeArrayImpl->reset();
						$groupAS->getAllGroups($p, $exec->getCrtModule(), $groupPTreeArrayImpl, $groupAS->getListFilterForSelectGroupWithoutDetail());
						...
						$limitFilterInGroup->setXml(simplexml_load_string($inGroupModuleXml));
					} else {
				...
				if ($_POST["action"] != null)
					...
					if($exec->getCrtAction() !="simpleFilters"){
						if($lc->getGroupPList()->count()>1){
							$limitFilterInGroupId = null;
						} else {
							$limitFilterInGroupId = reset($lc->getGroupPList()->getListIterator())->getId();
						}
						$filtersRec->setFieldValue($limitFilterInGroupId, "limitFilterInGroup");
					}
				}
				$form->ResolveForm($p, $exec, $state);
...
//							$exec->getCrtParameters(0) == "getGroupsContainingElements" ||
				...
				switch ($exec->getCrtParameters(0)) {
					...
					case "setGroupsContainingElement" :
						...
						$requestedSharing = array_combine($requestedSharing, $requestedSharing);
						$originalSharing = array_combine($originalSharing, $originalSharing);
					...
					case "setGroupsContainingElements" :
						...
						if($_POST["changeToSelected"]){
							...
							$changeToShare = array_combine($changeToShare, $changeToShare);
						...
						if($_POST["changeToUnselected"]){
							...
							$changeToUnshare = array_combine($changeToUnshare, $changeToUnshare);
					...
					case "manageEmails" :
						...
						foreach ($emailFieldsXml as $xmlField) {
							if($xmlField["maxExternalAccessLevel"]=="0"){
								throw new ServiceException("invalid multiple request. Cannot open external access if maxExternalAccessLevel is defined to 0 on " . $xmlField->getName(), ServiceException :: INVALID_ARGUMENT);
							}
							if($xmlField["maxExternalAccessLevel"]=="1"){
								$limitToRead = true;
							}
							$emailFields[] = $xmlField->getName();
						}
						//define the group id for config
						...
						try{
							switch ($for) {
								case "externalAccessMenuViewLink" :
									$elS->setMultipleExternalAccessForView($p, $lc->getMultipleSelection(), $emailFields, $endDate, $configGroupList);
									break;
								case "externalAccessMenuEditLink" :
									if($limitToRead){
										throw new ServiceException("invalid multiple request. Cannot open external access for Edit if maxExternalAccessLevel is defined to 1 on " . implode(",", $emailFields), ServiceException :: INVALID_ARGUMENT);
									}
									$elS->setMultipleExternalAccessForEdit($p, $lc->getMultipleSelection(), $emailFields, $endDate, $configGroupList);
									break;
								case "externalAccessMenuStop" :
									$elS->stopMultipleExternalAccess($p, $lc->getMultipleSelection(), $emailFields);
									break;
								default :
...
//						break;
					...
					case "setMultipleSelectionFolder" :
						...
						//$fsl->addElementAttributeSelector("id"); //filtering only on id make an error...
