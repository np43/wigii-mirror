...
foreach ($visibleLanguage as $idLang=>$languageLabel){
	...
	if($fieldXml["noWrap"]=="1") $tab .= 'noWrap ';
	...
	if((string)$fieldXml["wordlimit"]>0) $tab .='wordlimit wordlimit_'.(string)$fieldXml["wordlimit"].' ';
	if($fieldXml["height"]!=null) $tab .='difH ';
	$tab .= '" ';
	if($disabled || $readonly) $tab .=' disabled ';
	if($fieldXml["noWrap"]=="1") $tab .= ' wrap="off" ';
	$tab .=' style="width: 100%; max-width:'.($parentWidth).'px; overflow-y:auto;';
	if((string)$fieldXml["height"]!=null && (string)$fieldXml["height"] != "0") $tab .=' height:'.$fieldXml["height"].'px; ';
	else $tab .= 'max-height:'.$height.'px;';
