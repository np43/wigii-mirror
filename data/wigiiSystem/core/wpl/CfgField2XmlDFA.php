...
/**
 * Serializes cfgField StdClasses to XML following the Wigii configuration file syntax. 
 * Created by CWE on 02 Mars 2014
 */
class CfgField2XmlDFA implements DataFlowActivity
{	
	...
	private function debugLogger()
	{
		...
		{
			$this->_debugLogger = DebugLogger::getInstance("CfgField2XmlDFA");
		}
	...
	public function processDataChunk($data, $dataFlowContext) {
		...
		elseif($data instanceof stdClass) {
			$data = $this->getWplToolbox()->cfgField2Xml($data);
			if($this->groupInput) {				
				simplexml_appendChild($this->rootNode, $data);
			}
			else {
