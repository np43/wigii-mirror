...
' Modified by Medair September 2016 to make compatible with 64-bit and VBA7
...
' Returns:    a string with the Base64 encoded data.
#If VBA7 Then
Private Function Base64Encode2(InData() As Byte, ByVal InLen As Integer) As String
...
#End If
   ...
   Dim ODataLen As Integer: ODataLen = (InLen * 4 + 2) \ 3     ' output length without padding
   Dim OLen As Integer: OLen = ((InLen + 2) \ 3) * 4           ' output length including padding
   Dim Out() As Byte
   ReDim Out(0 To OLen - 1) As Byte
   Dim ip0 As Integer: ip0 = LBound(InData)
   Dim ip As Integer
   Dim op As Integer
...
Private Function Base64Decode(ByVal s As String) As Byte()
   ...
   Dim IBuf() As Byte: IBuf = ConvertStringToBytes(s)
      #If VBA7 Then ' 64 bits
      Dim ILen As LongPtr: ILen = UBound(IBuf) + 1
      #Else  ' 32 bits
   ...
   Do While ILen > 0
    ...
    #If VBA7 Then ' 64 bits
    Dim OLen As LongPtr: OLen = (ILen * 3) \ 4
    Dim Out() As Byte
    ReDim Out(0 To OLen - 1) As Byte
    Dim ip As LongPtr
    Dim op As LongPtr
...
Private Function ConvertStringToBytes(ByVal s As String) As Byte()
   Dim b1() As Byte: b1 = s
#If VBA7 Then ' 64 bits
   Dim l As Integer: l = (UBound(b1) + 1) \ 2
   If l = 0 Then ConvertStringToBytes = b1: Exit Function
   Dim b2() As Byte
   ReDim b2(0 To l - 1) As Byte
   Dim p As Integer
   For p = 0 To l - 1
      Dim c As Integer: c = b1(2 * p) + 256 * CInt(b1(2 * p + 1))
      If c >= 256 Then c = Asc("?")
      b2(p) = c
      Next
...
#If VBA7 Then ' 64 bits
   Dim l As Integer: l = UBound(b) - LBound(b) + 1
   Dim b2() As Byte
   ReDim b2(0 To (2 * l) - 1) As Byte
   Dim p0 As Integer: p0 = LBound(b)
   Dim p As Integer
...
Public Function Base64URLEncodeString(ByVal source As String) As String
    Base64URLEncodeString = VBA.Replace(VBA.Replace(VBA.Replace(Base64EncodeString(source), "+", "-"), "/", "_"), "=", "")
End Function

Public Function Base64URLDecodeString(ByVal source As String) As String
    source = Replace(Replace(source, "-", "+"), "_", "/") & "==="
    Base64URLDecodeString = Base64DecodeString(VBA.Left(source, VBA.Len(source) - (VBA.Len(source) Mod 4)))
End Function
