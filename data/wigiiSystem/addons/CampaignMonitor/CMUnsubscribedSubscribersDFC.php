...
/**
 * A data flow connector used to fetch unsubscribed subscribers into Campaign Monitor
 * Created by CWE on 12 décembre 2013
 */
class CMUnsubscribedSubscribersDFC implements DataFlowDumpable
{	
	...
	private function debugLogger() {
		if (!isset ($this->_debugLogger)) {
			$this->_debugLogger = DebugLogger :: getInstance("CMUnsubscribedSubscribersDFC");
		}
	...
	/**
	 * Sets the CS_REST_Lists instance to be used to fetch unsubscribed subscribers
	 ...
	 */
	public function setUnsubscribedSince($date) {
		if(is_int($date)) $this->startingDate = date('Y-m-d', $date);
		else $this->startingDate = $date;
	}
	protected function getUnsubscribedSince() {
	...
	public function dumpIntoDataFlow($dataFlowService, $dataFlowContext) {		
		...
		// gets first page
		$result = $this->executeGetUnsubscribedSubscribers($p);
		$p++;
		$nPages = $result->NumberOfPages;
		$this->debugLogger()->write("total number of selected unsubscribed subscribers : ".($result->TotalNumberOfRecords > 0 ? $result->TotalNumberOfRecords : 0));
		// dumps first page
	...
	private function executeGetUnsubscribedSubscribers($page) {
		$this->debugLogger()->write("fetches page #".$page);
		$result = $this->cs_REST_Lists->get_unsubscribed_subscribers($this->getUnsubscribedSince(),
			$page,
			$this->maxPageSize,
		...
		else throw new DataFlowServiceException("Error in getting unsubscribed subscribers on Campaign Monitor. Response is : ".json_encode($result), DataFlowServiceException::WRAPPING, $result->http_status_code);
	}
